...
#bcftools query -f '%CHROM\t%POS0\t%REF\t%ALT\n' -i 'TYPE!="snp"' ./single_chromosome/inputs/indels.vcf  | awk 'BEGIN {chr=""} {if (chr!=$1){chr=$1; balance=0; current_pos=0; previous_pos=0; print $1"\t"0"\t"0;}; diff=length($4)-length($3); current_pos=$2+1+balance; if (diff<0) print $1"\t"current_pos"\t"balance+diff; else for(i=1;i<=diff;i++){print $1"\t"current_pos+i"\t"balance+i}; balance=balance+diff; previous_pos=current_pos}' | sort -k 1,1 -k 2,2n > ./single_chromosome/outputs/chr_pos0_ref_alt.tsv
bcftools query -f '%CHROM\t%POS0\t%REF\t%ALT\n' ./single_chromosome/inputs/indels.vcf  | awk 'BEGIN {chr=""} {if (chr!=$1){chr=$1; balance=0; previous_pos=-1; print $1"\t"0"\t"0;}; if ($2<=previous_pos) next; diff=length($4)-length($3); current_pos=$2+1+balance; if (diff<0) print $1"\t"current_pos"\t"balance+diff; else for(i=1;i<=diff;i++){print $1"\t"current_pos+i"\t"balance+i}; balance=balance+diff; previous_pos=$2+length($3)-1}' | sort -k 1,1 -k 2,2n > ./single_chromosome/outputs/chr_pos0_ref_alt.tsv
